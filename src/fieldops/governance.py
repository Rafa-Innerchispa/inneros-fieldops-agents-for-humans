...
_ACTIONS = {
    ...
    "homeassistant.scene_turn_on": ActionPolicy(
        ...
        reversible=True,
        truth_note="Scene execution remains blocked until a per-scene allowlist and independent resulting-state verifier are bound.",
    ),
    "homeassistant.entity_control": ActionPolicy(
        action_type="homeassistant.entity_control",
        domain="facility_iot",
        description="Set one explicitly allowlisted Home Assistant light to on or off.",
        risk=RiskLevel.MEDIUM,
        requires_approval=True,
        availability=ActionAvailability.ENABLED_REAL,
        executor_id="inneros-ha-service-bridge",
        verifier_id="homeassistant-state-readback",
        reversible=True,
        truth_note="Real execution is available only for server-configured allowlisted lights and exact on/off state with independent HA readback; arbitrary entities and services remain unavailable.",
    ),
    ...
    "alarm.apply_state": ActionPolicy(
        ...
        reversible=True,
        truth_note="Alarm status/discovery exists; FieldOps does not enable arm/disarm/panic/siren/PGM until the dedicated critical-risk adapter is separately verified.",
    ),
