...
</style></head><body><div class="shell"><header><section class="hero"><div class="eyebrow">AWS Agents for Humans · InnerOS FieldOps</div><h1>Guided Judge Demo</h1><p>Live operational evidence across security, energy, network, alarm and telephony, followed by one human-governed action.</p><p><a href="__STATUS__">API status</a> · <a href="__LOGOUT__">Logout</a></p><p>UTC freshness: <code>__NOW__</code></p></section><section class="hero"><div class="eyebrow">Runtime health</div><div class="status-grid">__BADGES__</div></section></header>
<style>
...
.governed{background:linear-gradient(180deg,#111b28,#08111a);border-color:#336177;box-shadow:0 20px 55px rgba(0,0,0,.28)}
...
function runGovernedScenario(scenario){var title=byId('governed-title');var summary=byId('governed-summary');var technical=byId('governed-technical');var grid=byId('receipt-grid');var guide=byId('guide-6');if(guide){guide.classList.add('active');}title.textContent='Running governed workflow…';title.className='';summary.textContent='FieldOps is evaluating policy, human approval, bounded execution and independent verification.';grid.innerHTML='';fetch(demoUrl+'?scenario='+encodeURIComponent(scenario)).then(function(response){return response.json();}).then(function(data){var receipt=data&&data.receipt?data.receipt:null;technical.textContent=JSON.stringify(data,null,2);if(scenario==='denied'){title.textContent='DENIED — NOTHING EXECUTED';title.className='bad';summary.textContent='Human authority rejected the consequential action. The executor never ran. This is the fail-closed path.';grid.innerHTML=receiptItem('Decision','Denied by human')+receiptItem('Execution','Not started')+receiptItem('Safety','Fail-closed');return;}if(receipt&&receipt.quality_gate==='passed'){title.textContent='EXECUTED + VERIFIED';title.className='good';summary.textContent='Human approval was accepted, the bounded executor ran, and an independent verifier confirmed the expected state. Success is based on verified state, not a command ACK.';grid.innerHTML=receiptItem('Action',receipt.requested_action)+receiptItem('Target',receipt.target_ref)+receiptItem('Quality gate',receipt.quality_gate)+receiptItem('Executor',receipt.executor)+receiptItem('Verifier',receipt.verifier)+receiptItem('Correlation',receipt.correlation_id);setGuide(6,true);}else{title.textContent='NOT VERIFIED';title.className='warn';summary.textContent='The workflow did not pass the final verification gate. FieldOps refuses to call the action successful without independent proof.';grid.innerHTML=receipt?receiptItem('Action',receipt.requested_action)+receiptItem('Quality gate',receipt.quality_gate)+receiptItem('Verification',receipt.verification_passed):receiptItem('Status',(data&&data.status)||'blocked');}}).catch(function(error){title.textContent='REQUEST FAILED';title.className='bad';summary.textContent=String(error);technical.textContent=String(error);});}
