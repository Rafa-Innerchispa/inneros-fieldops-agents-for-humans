...
Public/browser access can inspect status and prepare proposals. Physical execution
is deliberately local-only: POST /api/execute is accepted only when explicitly
enabled and the request arrives directly through a loopback Host/client pair.
This prevents a public judge tunnel from becoming a home-control endpoint.
"""
...
from .operator_api import catalog_response, execute_response, propose_response
...
<style>
:root{{--bg:#05090f;--panel:#0b131d;--panel2:#111d2a;--line:#24384c;--text:#edf7ff;--muted:#91a7bb;--cyan:#42d7ff;--green:#55e49b;--amber:#ffc85a;--red:#ff7280}}
...
table{{width:100%;border-collapse:collapse;margin-top:12px;font-size:13px}}th,td{{text-align:left;padding:10px;border-bottom:1px solid var(--line);vertical-align:top}}td small{{display:block;color:var(--muted);margin-top:4px}}code{{color:#b7edff}}label{{display:block;color:var(--muted);font-size:12px;margin:10px 0 5px}}select,input,button,textarea{{width:100%;border:1px solid var(--line);background:#081521;color:var(--text);border-radius:9px;padding:10px;font:inherit}}button{{cursor:pointer;font-weight:800;margin-top:10px;background:#0c5e80}}button.secondary{{background:#142332}}pre{{white-space:pre-wrap;word-break:break-word;background:#03070b;border:1px solid #182637;border-radius:10px;padding:12px;max-height:350px;overflow:auto;color:#c2e9ce}}ul{{color:var(--muted)}}a{{color:var(--cyan)}}
@media(max-width:980px){{.grid{{grid-template-columns:1fr}}.flow{{grid-template-columns:1fr 1fr}}header{{flex-direction:column}}}}
</style>
</head>
<body><div class="shell">
<header><div><div class="eyebrow">InnerOS · production-shaped physical operations</div><h1>FieldOps <b>Operator Console</b></h1><p>One control plane for real-world actions. Reads are free; mutations are proposed, approved, bounded, independently verified and receipted.</p></div><div class="badges"><span class="badge good">HA REAL: {'BOUND' if ha_bound else 'NOT BOUND'}</span><span class="badge {'good' if dmx_bound else 'warn'}">DMX REAL: {'BOUND' if dmx_bound else 'NOT BOUND'}</span><span class="badge">PUBLIC EXECUTION: OFF</span></div></header>
<section class="panel" style="margin-top:18px"><div class="eyebrow">Universal action lifecycle</div><div class="flow"><div class="step"><b>01 Observe</b><small>Real telemetry/context</small></div><div class="step"><b>02 Propose</b><small>Central action registry</small></div><div class="step"><b>03 Approve</b><small>Human artifact required</small></div><div class="step"><b>04 Execute</b><small>Bounded adapter only</small></div><div class="step"><b>05 Verify</b><small>Independent readback</small></div><div class="step"><b>06 Receipt</b><small>Evidence + quality gate</small></div></div></section>
<div class="grid"><section class="panel"><div class="eyebrow">Action registry</div><h2>Current truth, not marketing fiction</h2><table><thead><tr><th>Action</th><th>Purpose</th><th>Risk</th><th>Runtime</th><th>Approval</th></tr></thead><tbody>{_action_rows(bundle)}</tbody></table></section>
<aside class="panel"><div class="eyebrow">Prepare a real proposal</div><h2>Home Assistant bounded control</h2><p>Only server-allowlisted noncritical lights can be targeted. This browser prepares the action. Physical execution remains on the trusted local operator path.</p><label>Target</label><select id="ha-target">{ha_options or '<option>No HA targets bound</option>'}</select><label>Desired state</label><select id="ha-state"><option value="on">on</option><option value="off">off</option></select><button id="propose-ha">Prepare proposal</button><label>Proposal / receipt</label><pre id="result">No proposal yet.</pre><p><a href="/judge">Open hackathon judge evidence view</a></p>{('<h3>Binding diagnostics</h3><ul>'+error_html+'</ul>') if error_html else ''}</aside></div>
<section class="panel" style="margin-top:16px"><div class="eyebrow">Real bindings</div><h2>{escape(', '.join(bundle.bindings) or 'No real adapters currently bound')}</h2><p>HTTP mutation is local-only and disabled unless explicitly enabled server-side. Public/judge access cannot manufacture an approval and operate physical devices.</p></section>
</div>
<script>
const result=document.getElementById('result');
document.getElementById('propose-ha')?.addEventListener('click', async()=>{{
 const body={{action_type:'homeassistant.entity_control',target_ref:document.getElementById('ha-target').value,parameters:{{state:document.getElementById('ha-state').value}},expected_state:{{state:document.getElementById('ha-state').value}}}};
 const r=await fetch('/api/propose',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(body)}}); result.textContent=JSON.stringify(await r.json(),null,2);
}});
</script></body></html>"""
...
class OperatorHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/api/status":
            self._json(200, {
                "ok": True,
                "operator_runtime": self.runtime_bundle.status_payload(),
                "inneros_context": read_inneros_context_snapshot(),
            })
            return
        ...
        if parsed.path == "/api/execute":
            if not _direct_local_execution_allowed(self):
                self._json(403, {
                    "ok": False,
                    "status": "blocked",
                    "error": "http_execution_not_local",
                    "message": "Physical HTTP execution is restricted to the explicitly enabled direct loopback operator path.",
                })
                return
